...
//this function filled the condos with the occupant and status attributes


// export async function addOccupantandStatus() {
//   console.log("hi");
//   try {
//
...
//
//       if (userData.owns) {
//         await Promise.all(userData.owns.map(async (condoId) => {
//           const condoDocRef = doc(db, "Condo", condoId);
...
//
//       if (userData.rents) {
//         await Promise.all(userData.rents.map(async (condoId) => {
//           const condoDocRef = doc(db, "Condo", condoId);
//           const condoDocSnap = await getDoc(condoDocRef);
//
//           if (condoDocSnap.exists) {
...
//this function filled the condos with the occupant and status attributes to empty if they dont exist
// export async function addDefaultOccupantStatus() {
//   const condosQuerySnapshot = await getDocs(collection(db, 'Condo'));
//
//   for (const condoDoc of condosQuerySnapshot.docs) {
//     const condoData = condoDoc.data();
//
//     if (!('occupant' in condoData)) {
//       await updateDoc(condoDoc.ref, { occupant: '' });
//     }
//
//     if ('status' in condoData && condoData.status === '') {
//       await updateDoc(condoDoc.ref, { status: 'Vacant' });
//     }
//   }
// }